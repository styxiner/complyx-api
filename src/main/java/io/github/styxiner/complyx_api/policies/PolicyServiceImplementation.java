...
/*
 * Servicio principal de gesti�n de Policies.
 *
 * RESPONSABILIDAD:
 * - Validar datos de entrada
 * - Orquestar l�gica de dominio
 * - Delegar mapping sin romper JPA
 *
 * NOTA:
 * La gesti�n de RegulationSection est� preparada pero comentada
 * hasta que el m�dulo correspondiente est� disponible.
 */
@Service
@Transactional
...
// FUTURO: integraci�n con regulation
	...
	public PolicyDetailDTO createPolicy(PolicyCreateDTO dto) {
		...
		PolicyEntity entity = policyMapperImplementation.toEntity(dto, new HashMap<>() // vac�o temporal FUTURO cambiar por
		// regulationMap
		);
	...
	public void deletePolicy(UUID policyId) {
		...
		/*
		 * IMPORTANTE: ManyToMany no siempre se limpia autom�ticamente de forma
		 * expl�cita en memoria, as� que rompemos relaciones para evitar
		 * inconsistencias.
		 */
...
//ASIGNACIONES
	...
	public void assignToGroup(UUID policyId, UUID groupId) {
		...
		/*
		 * Evita duplicados en la relaci�n ManyToMany.
		 */
	...
	/*
	 * activar cuando el m�dulo de regulations est� disponible
	 *
	 ...
	 *private Map<UUID, RegulationSectionEntity> loadRegulations(Set<UUID> ids) {
		...
		// Validaci�n importante: si faltan IDs, devolvemos error y no dejamos referencias rotas.
