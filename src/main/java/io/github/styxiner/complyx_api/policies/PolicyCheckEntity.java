...
public class PolicyCheckEntity {
	...
	/**
	 * Parámetros del check en formato JSON. Debe contener "type" + los parámetros
	 * específicos del executor del agente.
	 *
	 * Ejemplos: file_exists: { "type": "file_exists", "path":
	 * "/etc/ssh/sshd_config", "file_type": "file", "mode": "0600" } file_block: {
	 * "type": "file_block", "path": "/etc/pam.d/common-password", "must_contain":
	 * ["minlen=15"] } file_line: { "type": "file_line", "path": "/etc/login.defs",
	 * "key": "PASS_MIN_LEN", "operator": ">=", "value": "15" } ini_value: { "type":
	 * "ini_value", "path": "/etc/security/pwquality.conf", "key": "minlen",
	 * "operator": ">=", "value": "15" } file_absent: { "type": "file_absent",
	 * "path": "/etc/telnet.conf" } dir_contains: { "type": "dir_contains","path":
	 * "/etc/cron.d", "glob": "*.conf", "min_count": 1 } symlink: { "type":
	 * "symlink", "path": "/etc/localtime", "target": "/usr/share/zoneinfo/UTC" }
	 * pkg_installed:{ "type": "pkg_installed","name": "openssh-server", "version":
	 * "8.9", "operator": ">=" } pkg_absent: { "type": "pkg_absent", "name":
	 * "telnet" } service: { "type": "service", "name": "sshd", "active": true,
	 * "enabled": true } sysctl: { "type": "sysctl", "key": "net.ipv4.ip_forward",
	 * "operator": "=", "value": "0" } user_attr: { "type": "user_attr", "username":
	 * "root", "checks": [{"attr": "shell", "operator": "=", "value": "/bin/bash"}]
	 * }
	 */
